  ...
  .then((data) => {
    ...
    data.forEach((product) => {
      if (product.old_price) {
        ...
        const present_disc = Math.floor(
          ((product.old_price - product.price) / product.old_price) * 100
        );
        swiper_items_sale.innerHTML += ` <div class="swiper-slide product">
              <span class="sale_persent">%${present_disc}</span>
    ...
    data.forEach((product) => {
      if (product.catetory == "electronics") {
        ...
        const old_price_paragraph = product.old_price
          ? `<p class="old_price">$${product.old_price}</p>`
          : "";
        const present_disc_div = product.old_price
          ? `<span class="sale_persent">%${Math.floor(
              ((product.old_price - product.price) / product.old_price) * 100
            )} </span>`
          : "";
